...
export default async (req) => {
  ...
  const CSECRET = (process.env.PAYPAL_SECRET || "").trim();
  ...
  try {
    ...
    if (!ppAuth.ok) {
      let detail = "";
      try { const e = await ppAuth.json(); detail = e.error_description || e.error || ""; } catch (_) {}
      return json({ error: "Could not reach PayPal." + (detail ? " (" + detail + ")" : ""), status: ppAuth.status }, 502);
    }
