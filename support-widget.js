...
(function(){
  ...
  var SB_URL = 'https://tbmxxlngxyxprqfwyudm.supabase.co';
  var SB_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRibXh4bG5neHl4cHJxZnd5dWRtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODM2MzU1NTEsImV4cCI6MjA5OTIxMTU1MX0.MVjIfxsPQrsa5788gZt0Mu1BC9O2XEnaP6NcXv_Avyc';
  ...
  var CSS = ""
   ...
   + ".tsw-form{display:flex;flex-direction:column;gap:8px;margin-top:8px;}"
   + ".tsw-form input,.tsw-form textarea{border:1px solid var(--border);border-radius:10px;padding:9px 11px;font-family:var(--fb);font-size:14px;outline:none;background:#fff;color:var(--text);width:100%;}"
   + ".tsw-form textarea{resize:none;min-height:60px;}"
   + ".tsw-form button{background:var(--gold);color:#fff;border:none;border-radius:10px;padding:10px;font-family:var(--fb);font-weight:700;font-size:14px;cursor:pointer;}"
   + ".tsw-form button:hover{background:var(--rust);}"
   + ".tsw-form button:disabled{background:#B7BBAF;cursor:not-allowed;}"
  ...
  var HTML = ""
   ...
   +   '</div><button class="tsw-close" id="tsw-mail" aria-label="Leave a message" title="Leave a message" style="font-size:18px;line-height:1">\u2709</button><button class="tsw-close" id="tsw-close" aria-label="Minimize chat"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M6 6l12 12M18 6L6 18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></button></div>'
  ...
  function boot(){
    ...
    function handleSend(overrideText){
      ...
      if(/talk to a human|leave a message|contact (us|you)|speak (to|with) (a|someone)|human (help|please)/i.test(text)){
        appendBubble('user',text);apiHistory.push({role:'user',content:text});inputEl.value='';showContactForm();return;
      }
    ...
    var mailBtn=root.querySelector('#tsw-mail');
    if(mailBtn)mailBtn.addEventListener('click',function(){showContactForm();});

    function showContactForm(){
      hideQuickReplies();
      appendBubble('assistant',"Happy to get a teammate on it. Leave your name, email, and what's up — we'll follow up at the email you give.");
      var wrap=document.createElement('div');wrap.className='tsw-bubble tsw-bubble--assistant';
      wrap.innerHTML='<form class="tsw-form"><input id="tsw-c-name" type="text" placeholder="Your name" autocomplete="name"><input id="tsw-c-email" type="email" placeholder="Email" autocomplete="email"><textarea id="tsw-c-msg" placeholder="What do you need help with?"></textarea><button type="submit">Send message</button><div class="tsw-form-status" style="font-size:12px;color:var(--text-soft);min-height:14px"></div></form>';
      messagesEl.appendChild(wrap);scrollToBottom();
      var form=wrap.querySelector('form'), st=wrap.querySelector('.tsw-form-status');
      form.addEventListener('submit',function(e){
        e.preventDefault();
        var name=wrap.querySelector('#tsw-c-name').value.trim();
        var email=wrap.querySelector('#tsw-c-email').value.trim();
        var msg=wrap.querySelector('#tsw-c-msg').value.trim();
        if(!email||!msg){st.textContent='Please add your email and a message.';return;}
        var btn=form.querySelector('button');btn.disabled=true;st.textContent='Sending…';
        fetch(SB_URL+'/rest/v1/support_messages',{method:'POST',headers:{'Content-Type':'application/json','apikey':SB_ANON,'Authorization':'Bearer '+SB_ANON,'Prefer':'return=minimal'},
          body:JSON.stringify({name:name,email:email,message:msg,page:location.pathname})})
        .then(function(r){
          if(!r.ok)throw new Error('save');
          form.remove();
          appendBubble('assistant',"Got it, thanks"+(name?(', '+name):'')+"! We'll get back to you at "+email+" as soon as we can. Anything else I can help with in the meantime?");
        })
        .catch(function(){btn.disabled=false;st.textContent='Could not send — please email us at '+SUPPORT_EMAIL+' instead.';});
      });
    }
